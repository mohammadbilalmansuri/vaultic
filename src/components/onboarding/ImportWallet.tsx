...
const ImportWallet = ({ network, setStep }: ImportWalletProps) => {
  ...
  return (
    <motion.div {...scaleUpAnimation()} className="box max-w-xl">
      <h1 className="-mt-2 box-heading">Secret Recovery Phrase</h1>
      ...
      >
        <div className="w-full grid grid-cols-2 xs:grid-cols-3 gap-3">
          {Array.from({ length: mnemonicLength }, (_, index) => (
            <div
              key={index}
              className="w-full flex items-center gap-2 p-3 rounded-2xl transition-all duration-300 bg-primary focus-within:bg-secondary"
            >
