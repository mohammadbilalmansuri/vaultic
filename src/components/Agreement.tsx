...
export default function Agreement({ setStep }: AgreementProps) {
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-medium heading-color text-nowrap">
        Secret Recovery Phrase Warning
      </h1>
      <p className="pb-2 -mt-1 max-w-sm">
        On the next page, you will receive your secret recovery phrase.
      </p>

      <div className="flex items-center gap-4 text-left px-5 py-4 rounded-xl dark:bg-zinc-800 bg-zinc-200 dark:border-zinc-700 border-zinc-300 border-1.5">
        <svg
      ...
      <div
        className="flex items-center gap-4 text-left py-2 cursor-pointer select-none"
        onClick={() => setAgree((prev) => !prev)}
      >
        <div className="min-w-fit">
          <Switch state={agree} isStateDependentColor={true} />
        </div>
        <p>
